...
class IntervalTest extends RepositoryTest {
    ...
    class LONG_INTERVAL_TEST {
        ...
        void simpleExample(int interval, int count) {
            ...
            //테스트에 필요한 데이터를 준비합니다.
            Member member = em.createQuery("select m from Member m where m.id = 1L", Member.class)
                .getSingleResult();
            Category category = em.createQuery(
                    "select c from Category c "
                        + " where c.member = :member "
                        + " and c.orderInterval = :interval", Category.class)
            ...
            List<Item> items = new ArrayList<>(em.createQuery("select i from Item i where i.id <= 4L order by i.id",
                    Item.class)
                .getResultList());
            ...
            ModifyOrderIntervalDto request = ModifyOrderIntervalDto.builder()
                .member(member)
                .category(category)
            ...
            log.info("변경 후 결과값");
            List<FavoriteItemWithInterval> resultList = new ArrayList<>(em.createQuery(
